...
If turned on, SELECT queries may utilize the [query result cache](../query-cache.md). Parameters [use_query_cache_passive_usage](#use-query-cache-passive-usage)
and [use_query_cache_active_usage](#use-query-cache-active-usage) control how the cache can be used in more detail.
